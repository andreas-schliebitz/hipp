...
def n_jobs() -> int:
    n = os.environ["HIPP_JOBS"] if "HIPP_JOBS" in os.environ else 1
    try:
        n = int(n)
    except:
        n = 1
...
def invertible_cov(X: np.ndarray) -> np.ndarray:
    ...
    if np.linalg.cond(C) >= 1 / np.finfo(C.dtype).eps:
        logging.info("Using Ledoit-Wolf to create invertible covariance matrix.")
        C, _ = ledoit_wolf(X)
...
def zstd(
    ...
    assert (input_filepath or input_data is not None) and (
        mode == "decompress" or mode == "compress"
    )
    ...
    if input_filepath:
        if output_filepath:
            ...
            if mode == "compress":
                ...
                # Decompress input file into byte array
                with lz4.frame.open(input_filepath, mode="rb") as ih:
                    return ih.read()
    else:
        if not isinstance(input_data, bytes):
            input_bytes = (
